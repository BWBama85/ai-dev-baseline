...
Read on demand from step 8. The prompt itself is **built by `bash "$HOME/.claude/scripts/lib/implement-lib.sh" dispatch-review`**
...
omitted. `bash "$HOME/.claude/scripts/lib/implement-lib.sh" review-verdict <file>` is the one reader; `dispatch-review` calls it
...
`bash "$HOME/.claude/scripts/lib/role-dispatch.sh" review-rung claude` decides it once, over three readers
...
Resolve once — `bash "$HOME/.claude/scripts/lib/role-dispatch.sh" effort review` — and pass `--effort` to every slot, because
...
2. **Adversarial bug review** — a Claude subagent (Agent tool, `general-purpose`) over the fresh
   ...
   **Then publish its reply — do not consume it directly** (#488):
   ...
   ```bash
   bash "$HOME/.claude/scripts/lib/implement-lib.sh" publish-review [--slot N] .claude/state   # the subagent's reply on stdin
   ```
