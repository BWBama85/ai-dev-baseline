...
native. For each resolved token: if it equals `claude` (the agent driving
this run) run that agent's review **in-process** (below); otherwise shell it out with
`bash "$HOME/.claude/scripts/lib/role-dispatch.sh" invoke <token>` over the diff. **Every configured reviewer is a
...
- `codex` → `bash "$HOME/.claude/scripts/lib/role-dispatch.sh" invoke codex` over a review prompt on the diff (it runs
...
- `gemini` → `bash "$HOME/.claude/scripts/lib/role-dispatch.sh" invoke gemini` over the diff (it runs `agy -p`).
