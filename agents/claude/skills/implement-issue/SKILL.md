...
"$HOME/.claude/skills"/implement-issue/): `state-protocol.md` — marker/claim/owner contracts in
...
.claude/state <n>…`, dispatch the subagent over it, and write its returned summary to
`.claude/state/survey.md` yourself. **Otherwise**, one bounded background call:
...
- `TaskCreate` 3–8 tracked sub-tasks. Read `survey.md` first; read code before
