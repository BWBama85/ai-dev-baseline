...
**Every call below passes `--state .claude/state`, and that is not boilerplate.** Without it the
...
bash "$HOME/.claude/scripts/lib/docs-lib.sh" probe-record --state .claude/state --server <name> --result usable|degraded|absent \
  --evidence '<what you observed — the call and its result, not "it worked">'

bash "$HOME/.claude/scripts/lib/docs-lib.sh" verdict --state .claude/state; VRC=$?
case "$VRC" in
...
```bash
bash "$HOME/.claude/scripts/lib/docs-lib.sh" consulted --state .claude/state --surface '<the API/service/flag>' --rung <1|2|3> \
...
```bash
bash "$HOME/.claude/scripts/lib/docs-lib.sh" none-needed --state .claude/state --justification '<why every surface here is trivial or already-proven>'
```
...
```bash
bash "$HOME/.claude/scripts/lib/docs-lib.sh" report --state .claude/state; DRC=$?
case "$DRC" in
...
**State the documentation disposition, in one line (#422).** Render it with `bash "$HOME/.claude/scripts/lib/docs-lib.sh" report --state .claude/state`
