...
collide — so a second run is **refused**, not accommodated. Preflight asks `bash "$HOME/.claude/scripts/lib/implement-lib.sh" admit`
...
**"Per agent" is not a footnote.** The scope is this agent's `.claude/state`, so what is enforced is
...
- **`review`** (shipped default `["codex"]`) — the code-review agents in step 8. Always
  ...
  fallback for an unset `review` is still the primary's own pass (`bash "$HOME/.claude/scripts/lib/role-dispatch.sh"
...
`bash "$HOME/.claude/scripts/lib/role-dispatch.sh"` (`scripts/lib/role-dispatch.sh`) does the resolution + cross-agent
shelling for you — `bash "$HOME/.claude/scripts/lib/role-dispatch.sh" resolve <role>` prints the token(s);
`bash "$HOME/.claude/scripts/lib/role-dispatch.sh" invoke <role|agent>` (prompt on stdin) runs one agent's CLI and
...
- `codex` → `codex exec --cd <repo> -`; `gemini` → `agy -p`. `bash "$HOME/.claude/scripts/lib/role-dispatch.sh" invoke`
...
bash "$HOME/.claude/scripts/lib/implement-lib.sh" admit .claude/state; ADMIT=$?
case "$ADMIT" in
...
for n in "${ISSUE_NUMS[@]}"; do
  gh issue view "$n" --json number,title,body,labels,author,comments,milestone,state > "/tmp/issue-$n.json" \
    || { bash "$HOME/.claude/scripts/lib/implement-lib.sh" release .claude/state; echo "ERROR: issue #$n not found in this repo — verify repo scope"; exit 1; }
done
...
for n in "${ISSUE_NUMS[@]}"; do
  gh api "repos/{owner}/{repo}/issues/$n" --jq '.author_association' > "/tmp/issue-$n.assoc" \
    || { bash "$HOME/.claude/scripts/lib/implement-lib.sh" release .claude/state; echo "ERROR: could not read #$n's author association"; exit 1; }
done
```
...
for n in "${ISSUE_NUMS[@]}"; do
  ...
  [ "$st" = "OPEN" ] || { bash "$HOME/.claude/scripts/lib/implement-lib.sh" release .claude/state
...
for n in "${ISSUE_NUMS[@]}"; do
  ...
  TEXT="$(jq -r --arg assoc "$(cat "/tmp/issue-$n.assoc")" '
      ...
      | join("\n\n---\n\n")' "/tmp/issue-$n.json")" \
    || { bash "$HOME/.claude/scripts/lib/implement-lib.sh" release .claude/state; echo "ERROR: could not read issue #$n's text"; exit 1; }
  printf '%s' "$TEXT" | bash "$HOME/.claude/scripts/lib/role-dispatch.sh" untrusted "github-issue #$n" >> .claude/state/gap-prompt.txt \
    || { bash "$HOME/.claude/scripts/lib/implement-lib.sh" release .claude/state; echo "ERROR: could not contain issue #$n's text — do NOT fall back to pasting it raw"; exit 1; }
done
...
- **On any path that STOPS the run here** — a BLOCKING finding you surface, or a gap-analysis
  ...
  ```bash
  bash "$HOME/.claude/scripts/lib/implement-lib.sh" release .claude/state
  ```
...
`bash "$HOME/.claude/scripts/lib/role-dispatch.sh" resolve review` — it prints one token per slot. **Do not**
...
`bash "$HOME/.claude/scripts/lib/state-assert.sh" observe pr <N>` (`base/practices/verify-before-asserting.md`).
...
- **A refusal that does NOT clear itself** → some do not, and the message says which. An abandoned
  ...
  on `.claude/state`. Each refusal prints the recovery: finish the branch, run `/cleanup`, or delete
...
- **Stop hook nags about a run you never started** → it is reading a marker owned by
  ...
  `jq -r '.owner, .branch' .claude/state/implement-issue-active.json`: an `owner` that
