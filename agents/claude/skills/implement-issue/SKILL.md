...
.claude/state <n>…`, dispatch the subagent over the path its `prompt-ready` line prints (a
...
`bash "$HOME/.claude/scripts/lib/implement-lib.sh" publish-review --slot N .claude/state` — **the same slot number the
...
`bash "$HOME/.claude/scripts/lib/implement-lib.sh" read-artifact .claude/state review`, and `review-<n>` for each additional
...
IDENT="$(bash "$HOME/.claude/scripts/lib/implement-lib.sh" sweep-identity .claude/state)" \
...
bash "$HOME/.claude/scripts/lib/pattern-ledger.sh" rule-sweep --state .claude/state --run "$RS_RUN" --tree "$RS_TREE" \
...
- **Learned-checklist sweep** (#421, #490): render it — never write the sentence by hand, which is
  ...
  ```bash
  bash "$HOME/.claude/scripts/lib/pattern-ledger.sh" rule-sweep-report --state .claude/state --run "$RS_RUN" --tree "$RS_TREE"
  ```
