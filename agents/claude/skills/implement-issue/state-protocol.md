...
`bash "$HOME/.claude/scripts/lib/implement-lib.sh"` (`admit` / `release` / the step subcommands) is the mechanism, and
...
than accommodated. Preflight asks `bash "$HOME/.claude/scripts/lib/implement-lib.sh" admit .claude/state` before anything is
...
Stage every marker write inside `.claude/state/` (`.marker.tmp` → `mv`) so the rename is atomic.
...
**inline** with the literal token (`bash "$HOME/.claude/scripts/lib/implement-lib.sh" release --token <token> .claude/state`) —
...
line, the claim carries it: `jq -r .token .claude/state/gap-analysis.lock`.
...
`.claude/state` is repo-relative and per-agent, which is the boundary `admit` already enforces; a
