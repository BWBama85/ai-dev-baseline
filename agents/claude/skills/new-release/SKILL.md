...
# issues, never edited). That produced issue-tracker debt for one-line tweaks and
# a dead "monitor" bucket nobody revisited. The model is now: for a CLI we track
# closely, the output of a release review is a CHANGE (a PR or a user-config
# edit), not a backlog item. Edit/Write are allowed because the skill now applies
# fixes. It still asks before mutating anything (single approval gate, Step 6).
