...
for n in $(printf '%s' "$CAND_JSON" | jq -r '.[].number'); do
  ...
  for d in $(printf '%s' "$BODY" | bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" deps-from-body "$n"); do
    printf '[%s,%s]\n' "$n" "$d" >> "$CDIR/edges"
  done
done
...
printf '%s\n' "$DEC_SECTION" | grep '^[[:space:]]*|' | while IFS= read -r row; do
  ...
  for d in $(printf '%s' "$DCELL" | bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" deps-from-body "$Q"); do
    printf '[%s,%s]\n' "$Q" "$d" >> "$CDIR/edges"
  done
done
