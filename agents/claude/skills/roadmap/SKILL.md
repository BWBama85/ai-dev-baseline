...
re-derived in prose: `bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" emit-verdict` takes the still-open members' classification
...
EOF
  ...
  HEALTH_OUT="$(printf '%s' "$HEALTH_IN" | bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" branch-health "$HEAD_SHA" "$WF_COUNT" "$HEALTH_DECL")" \
...
- `not-green` → requirements are met but the branch is **red**. Withhold the cut and name the
  ...
  CI_CLASS=0
  if [ -n "$CI_RUN" ]; then
    bash "$HOME/.claude/scripts/lib/ci-health.sh" classify --run "$CI_RUN" >/dev/null 2>&1; CI_CLASS=$?
  fi
  ```
...
- **Re-derive every dependency edge from its source — never carry one forward.** The
  ...
  DAMB="$(printf '%s' "$DCELL" | bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" deps-ambiguous "$Q")" \
...
2. **Check the recorded set before surfacing anything.** A question whose id already appears in
   ...
   ```bash
   ANSWERED="$(printf '%s' "$ARTIFACT_BODY" | bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" decisions)" \
...
1. **Dependencies first.** Never place a bundle before a bundle it depends on. The edge set is
   the one step 4 **re-derived this run** via `bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" deps-from-body` — explicit
...
while IFS= read -r row; do
  ...
  DDEPS="$(printf '%s' "$DCELL" | bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" deps-from-body "$Q")" \
...
printf '%s' "$UNIV_JSON" \
  ...
  | bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" compose-candidates "$ROADMAP_NUM" bug > "$CDIR/cand.tsv" \
...
bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" compose-select < "$CDIR/cand.tsv" > "$CDIR/plan" \
...
case "$(bash "$HOME/.claude/scripts/lib/roadmap-lib.sh" read-complete "$GOT" "$EXPECTED")" in
  complete) : ;;
