...
while IFS="$TABC" read -r kind path key; do
  ...
  IDENT="$(bash "$HOME/.claude/scripts/lib/cleanup-lib.sh" marker-identity "$path")"
...
"; RUN=keep; continue
  ...
  if [ "$MV" = stale ]; then
    ...
    NOW="$(bash "$HOME/.claude/scripts/lib/cleanup-lib.sh" marker-identity "$path")"
...
while IFS="$TABC" read -r kind path key; do
  case "$kind" in
    gaps)
      ...
      ;;
    threads)
      TV="$(bash "$HOME/.claude/scripts/lib/cleanup-lib.sh" state-verdict threads "$(pr_state "$key")")" || continue
      [ "$TV" = stale ] || continue
      sweep_file "$path"
      ;;
  esac
