...
LV="$(bash "$HOME/.claude/scripts/lib/cleanup-lib.sh" state-verdict rules "$RUN_NOW")" || LV=keep
...
bash "$HOME/.claude/scripts/lib/cleanup-lib.sh" run-live "$STATE"; RL_RC=$?
...
while IFS="$TABC" read -r kind sfile key ident; do
  case "$kind" in
    ...
    sweep)
      ...
      PRST="$(pr_state "$key")"
      WV="$(bash "$HOME/.claude/scripts/lib/cleanup-lib.sh" state-verdict sweep "$PRST")" || continue
      [ "$WV" = stale ] || continue
      sweep_file "$sfile" "$ident" "PR $PRST"
      ;;
  esac
