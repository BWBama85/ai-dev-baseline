...
if [ "$VERDICT" = "met" ]; then
  ...
  HEALTH_OUT="$(printf '%s' "$HEALTH_IN" | bash "$HOME/.gemini/scripts/lib/roadmap-lib.sh" branch-health "$HEAD_SHA" "$WF_COUNT")" \
  ...
  case "$CMD" in
    "$PFX"?*)
      ...
      if [ -n "$SKILL_NAME" ] && [ "$RESOLVES" -eq 0 ] && [ -z "$PROBE" ]; then
        ...
        # and reports a present skill as missing.  is a LIST (Codex loads both
...
for n in $(printf '%s' "$CAND_JSON" | jq -r '.[].number'); do
  ...
  for d in $(printf '%s' "$BODY" | bash "$HOME/.gemini/scripts/lib/roadmap-lib.sh" deps-from-body "$n"); do
    printf '[%s,%s]\n' "$n" "$d" >> "$CDIR/edges"
  done
done
...
printf '%s\n' "$DEC_SECTION" | grep '^[[:space:]]*|' | while IFS= read -r row; do
  ...
  for d in $(printf '%s' "$DCELL" | bash "$HOME/.gemini/scripts/lib/roadmap-lib.sh" deps-from-body "$Q"); do
    printf '[%s,%s]\n' "$Q" "$d" >> "$CDIR/edges"
  done
done
