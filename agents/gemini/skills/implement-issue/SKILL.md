...
`bash "$HOME/.gemini/scripts/lib/implement-lib.sh" admit` before it deletes anything; the run then holds a **run claim**
...
**Scope is this agent's `.gemini/state`**, so what is enforced is a second run of the *same*
...
Stage every marker write inside `.gemini/state/` (`.marker.tmp` → `mv`) so the rename is
...
- **`review`** (shipped default `["codex"]`) — the code-review agents in step 8. Always also do
  ...
  fallback for an unset `review` is still the primary's own pass (`bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" resolve
  review`), so a repo with no manifest at all is unchanged.

Resolve tokens to invocations via `base/roles.md`. The runtime helper `bash "$HOME/.gemini/scripts/lib/role-dispatch.sh"`
(`scripts/lib/role-dispatch.sh`) does the resolution and cross-agent shelling:
`bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" resolve <role>` prints the token(s); `bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" invoke <role|agent>`
...
- `codex` → `codex exec --cd <repo> -`; `gemini` → `agy -p`. `bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" invoke` wraps
...
cannot see that line, the claim carries it: `jq -r .token .gemini/state/gap-analysis.lock`.
...
**The snapshot lands in `.gemini/state`, never in `/tmp`, and FLAT rather than in a
...
they are read back minutes later in steps 3 and 8. `.gemini/state` is repo-relative and
...
directly under the state directory — a tidy-looking `.gemini/state/issues/` is invisible to
`/cleanup` and to `admit` alike.

**`.gemini/state` must be gitignored before any of it is written.** A repo initialized before
...
```bash
# ASK ABOUT THE FILES, not about the directory. `git check-ignore .gemini/state` answers 1 —
...
**UNTRUSTED READ SITE — `.gemini/state/issue-<n>.json`.** Its `body` and every
...
CHECKLIST="$(bash "$HOME/.gemini/scripts/lib/pattern-ledger.sh" checklist 2>/dev/null || true)"
if [ -n "$CHECKLIST" ]; then
  {
...
`.gemini/state`, where step 5's marker is what proves the run is live.

#### 5b-i. The MCP preflight — only for servers this repo DECLARES

```bash
REQUIRED="$(bash "$HOME/.gemini/scripts/lib/docs-lib.sh" mcp-required)"; MRC=$?
case "$MRC" in
...
| the async reviewer on the PR | **a library wait** — `bash "$HOME/.gemini/scripts/lib/pr-watch.sh" wait`, driven by `/resolve-pr-threads`, whose step 0b specifies the dispatch |
| the project's gates | **a blocking command** — `bash "$HOME/.gemini/scripts/lib/project-gates.sh" run`, above. Fix and re-run |
...
```bash
bash "$HOME/.gemini/scripts/lib/pattern-ledger.sh" checklist; CRC=$?
case "$CRC" in
...
Then run each configured `review` agent. Resolve the slots with `bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" resolve
...
```bash
bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" available <token>   # 0 = CLI on PATH · 1 = known agent, CLI absent · 2 = not a token
bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" review-rung gemini   # the whole ladder, decided once (rc 2 = unknown)
...
issue status in the close-out comes from `bash "$HOME/.gemini/scripts/lib/state-assert.sh" observe pr <N>`
...
- **A claim survives a killed run** → its lease (9000s / 2h30m) expires and the next run breaks it
  ...
  `.gemini/state/gap-analysis.lock` is the documented escape.
- **A refusal that does NOT clear itself** → some do not, and the message says which. An abandoned
  ...
  permissions on `.gemini/state`. Each refusal prints its recovery: finish the branch, run
...
- **Stop hook nags about a run you never started** → it is reading a marker owned by another
  ...
  .gemini/state/implement-issue-active.json`: an `owner` that is not your session id means the
