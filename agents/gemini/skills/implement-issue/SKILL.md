...
CLEAR_RC=0
find .gemini/state -maxdepth 1 \( -type f -o -type l \) \
     \( -name 'review-*.md' -o -name 'review-*.err' \) -exec rm -f {} + || CLEAR_RC=$?
rm -f .gemini/state/review-prompt.txt .gemini/state/review.md .gemini/state/review.err || CLEAR_RC=$?
[ "$CLEAR_RC" -eq 0 ] || { echo "ERROR: could not clear stale review artifacts from .gemini/state"; exit 1; }
```
