...
rm -f .gemini/state/review-prompt.txt .gemini/state/review.md .gemini/state/review.err
...
# `.gemini/state/review-*.md` to the `rm -f` above looks equivalent and is not — an unmatched glob
...
find .gemini/state -maxdepth 1 -type f \( -name 'review-*.md' -o -name 'review-*.err' \) \
     -exec rm -f {} +
```
