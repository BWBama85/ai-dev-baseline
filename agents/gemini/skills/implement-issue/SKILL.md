...
so a second run is **refused**, not accommodated. Preflight asks `bash "$HOME/.gemini/scripts/lib/implement-lib.sh" admit` before it
...
different `.gemini/state`s and never collide on these paths at all; they still share HEAD, and the
...
- **`review`** (shipped default `["codex"]`) — the code-review agents in step 8. Always
  ...
  fallback for an unset `review` is still the primary's own pass (`bash "$HOME/.gemini/scripts/lib/role-dispatch.sh"
...
stop_run() { bash "$HOME/.gemini/scripts/lib/implement-lib.sh" release .gemini/state; echo "$1"; exit 1; }
```
...
`bash "$HOME/.gemini/scripts/lib/state-assert.sh" observe pr <N>` (`base/practices/verify-before-asserting.md`).
...
- **A claim survives a killed run** → its lease (twice the dispatch backstop plus an hour) expires
  ...
  you are certain the run is dead, deleting `.gemini/state/gap-analysis.lock` is the documented
  escape.
