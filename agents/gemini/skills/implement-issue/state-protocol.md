<!-- GENERATED FILE — do not edit by hand.
     Source: base/workflows/implement-issue/state-protocol.md · Regenerate: scripts/build.sh
...
`bash "$HOME/.gemini/scripts/lib/implement-lib.sh"` (`admit` / `release` / the step subcommands) is the mechanism, and
...
than accommodated. Preflight asks `bash "$HOME/.gemini/scripts/lib/implement-lib.sh" admit .gemini/state` before anything is
...
Stage every marker write inside `.gemini/state/` (`.marker.tmp` → `mv`) so the rename is atomic.
...
**inline** with the literal token (`bash "$HOME/.gemini/scripts/lib/implement-lib.sh" release --token <token> .gemini/state`) —
...
line, the claim carries it: `jq -r .token .gemini/state/gap-analysis.lock`.
...
run), and SERIALIZED with `admit`'s expired-claim reap through a shared mutex under which both
sides re-read before acting. A foreign token, an already-lapsed own lease, and an ABSENT claim
(a successor may have completed admission and released at its marker) all refuse the
subcommand (13) rather than race a successor — so the 9000s bounds each step and the
...
`.gemini/state` is repo-relative and per-agent, which is the boundary `admit` already enforces; a
