<!-- GENERATED FILE — do not edit by hand.
     Source: base/workflows/implement-issue/dispatch-failures.md · Regenerate: scripts/build.sh
...
`bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" invoke` runs the resolved agent's CLI under a **45-minute (2700 s) hang
...
share of the claim lease, with margin) because a survey that
...
- **A claim survives a killed run** → its lease (9000s) expires and the next run breaks it with a
  NOTE. Waiting is correct; deleting `.gemini/state/gap-analysis.lock` is the documented escape
  when you are certain the run is dead.
- **A refusal that does NOT clear itself** → an abandoned marker whose branch ref survives, a
  malformed marker, a persistent `gh` failure, wrong permissions on `.gemini/state`. Each refusal
...
- **The Stop hook nags about a run you never started** → it is reading a marker owned by another
  ...
  `jq -r '.owner, .branch' .gemini/state/implement-issue-active.json`: an `owner` that is not
