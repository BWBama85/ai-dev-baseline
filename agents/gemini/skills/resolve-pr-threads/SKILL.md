...
if [ "$WATCH" = "1" ]; then
  ...
  # bounds calls the library directly (bash "$HOME/.gemini/scripts/lib/pr-watch.sh" wait --pr N --interval S --max-secs S)
  # and then runs this skill without --watch.
  bash "$HOME/.gemini/scripts/lib/pr-watch.sh" wait --pr "$PR_NUM"; WRC=$?
else
...
}' -f owner="$OWNER" -f repo="$REPO" -F num="$PR_NUM" > .gemini/state/threads-$PR_NUM.json
```
...
```bash
TOTAL=$(jq '[.data.repository.pullRequest.reviewThreads.nodes[] | select(.isResolved==false)] | length' .gemini/state/threads-$PR_NUM.json)
...
For every unresolved thread, read it with the Read tool (load `.gemini/state/threads-$PR_NUM.json`) and decide one of:
...
: "${BOT_RE:=(?i)^($(bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" bots | sed 's/[][]/\\&/g' | paste -sd'|' -))$}"
