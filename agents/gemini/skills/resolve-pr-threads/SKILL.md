...
REVIEW_TOKEN="$(bash "$HOME/.gemini/scripts/lib/role-dispatch.sh" resolve review | head -n 1)"
...
case "$RUNG" in
  independent*|same-model*)
    bash "$HOME/.gemini/scripts/lib/implement-lib.sh" dispatch-sweep ${EFFORT:+--effort "$EFFORT"} --pr "$PR_NUM" --findings "$FINDINGS" .gemini/state "$REVIEW_TOKEN"; SWRC=$? ;;
  deferred*|none*)
    bash "$HOME/.gemini/scripts/lib/implement-lib.sh" dispatch-sweep --skipped --pr "$PR_NUM" --findings "$FINDINGS" .gemini/state; SWRC=$? ;;
