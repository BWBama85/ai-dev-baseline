...
case "$RUNG" in
  independent*|same-model*)
    bash "$HOME/.gemini/scripts/lib/implement-lib.sh" dispatch-review ${EFFORT:+--effort "$EFFORT"} --criteria-from-pr "$PR_NUM" .gemini/state "$REVIEW_TOKEN" ;;
...
```bash
bash "$HOME/.gemini/scripts/lib/implement-lib.sh" read-artifact .gemini/state review
```
...
sweep_once() {
  case "$RUNG" in
    independent*|same-model*)
      bash "$HOME/.gemini/scripts/lib/implement-lib.sh" dispatch-sweep ${EFFORT:+--effort "$EFFORT"} --pr "$PR_NUM" --findings "$FINDINGS" .gemini/state "$REVIEW_TOKEN" ;;
    deferred*|none*)
      bash "$HOME/.gemini/scripts/lib/implement-lib.sh" dispatch-sweep --skipped --pr "$PR_NUM" --findings "$FINDINGS" .gemini/state ;;
...
```bash
bash "$HOME/.gemini/scripts/lib/implement-lib.sh" sweep-mark "$SWEEP_FILE" --class <class> --site <path[:line]> --result fixed --fix "$FIX_SHA"
bash "$HOME/.gemini/scripts/lib/implement-lib.sh" sweep-mark "$SWEEP_FILE" --class <class> --site <path[:line]> --result deferred --issue <n>
bash "$HOME/.gemini/scripts/lib/implement-lib.sh" sweep-mark "$SWEEP_FILE" --class <class> --site <path[:line]> --result declined --reason '<why it differs>'
```
...
- **EVERY result except `0` and `10` STOPS THE ROUND — do not continue to step 5.** A hit that was
  ...
  | `18` | the ledger, or the sweep file, does not parse | **stop**; `bash "$HOME/.gemini/scripts/lib/pattern-ledger.sh" verify` names a ledger record |
