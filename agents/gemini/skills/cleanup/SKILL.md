...
- **The preamble is part of the report.** Every branch/state line below is already rendered from a
  ...
  comes from `bash "$HOME/.gemini/scripts/lib/state-assert.sh" observe pr|issue <n>`, per
...
- **So: a third field on the report record**, `<category>TAB<item>TAB<proof>`, rendered by
  `bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" report`. A record with no third field renders exactly as it did before, so
...
- **Never a `#N` beside a status word.** `bash "$HOME/.gemini/scripts/lib/state-assert.sh" lint` rejects that shape unless it is
...
if ! SCAN="$(bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" state-scan "$STATE")"; then
...
while IFS="$TABC" read -r kind sfile key; do
  ...
  IDENT="$(bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" marker-identity "$sfile")"
...
"; RUN=keep; continue
  ...
  if [ "$MV" = stale ]; then
    ...
    NOW="$(bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" marker-identity "$sfile")"
...
if ! SCAN="$(bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" state-scan --with-identity "$STATE")"; then
...
bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" run-live "$STATE"; RL_RC=$?
...
sweep_file() {
  local now
  now="$(bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" file-identity "$1")"
...
while IFS="$TABC" read -r kind sfile key ident; do
  case "$kind" in
    ...
    threads)
      ...
      PRST="$(pr_state "$key")"
      TV="$(bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" state-verdict threads "$PRST")" || continue
      [ "$TV" = stale ] || continue
...
"
    ...
    other)
      ...
      [ "$(bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" marker-shape "$sfile")" = marker-shaped ] || continue
...
while IFS="$TABC" read -r kind sfile key ident; do
  ...
  # A bounded filename open (bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" file-size), never a parent-shell redirect: these
  ...
  sz="$(bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh" file-size "$sfile" 2>/dev/null)" || sz=""
...
`bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh"`, and step 7 may replace that very library on disk (it re-runs the installer,
whose symlinks are what `bash "$HOME/.gemini/scripts/lib/cleanup-lib.sh"` resolves through). Composing the report first means the
...
`bash "$HOME/.gemini/scripts/lib/state-assert.sh" lint` rejects a status word sharing a sentence with an entity reference,
...
Every decision is `bash "$HOME/.gemini/scripts/lib/currency-lib.sh"`'s: the mode (`[updates] session_start` in the global
...
CU_RECORD="$(bash "$HOME/.gemini/scripts/lib/currency-lib.sh" check --trigger cleanup 2>/dev/null || true)"
...
- Run it after a merge, or periodically. It is idempotent — a second run finds nothing new and
  ...
  check is never suppressed by the rate limit — see `bash "$HOME/.gemini/scripts/lib/currency-lib.sh"`).
