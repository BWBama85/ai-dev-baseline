...
```bash
# ADB-SNIPPET: resolve
# THROUGH bash "$HOME/.gemini/scripts/lib/adopt-lib.sh", never by sourcing an agent's common.sh directly. This skill renders for
...
# resolution (base/practices/repo-scope.md) — and `bash "$HOME/.gemini/scripts/lib/adopt-lib.sh"` is already mapped per agent.
#
...
SHAPE="$(bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" resolve "${TARGET:-$PWD}")"
printf '%s' "$SHAPE" | bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" shape-field warning | while IFS= read -r m; do
  [ -n "$m" ] && echo "warning: $m" >&2
done
PROJECT="$(printf '%s' "$SHAPE" | bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" shape-field root)"
...
bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" scan "$PROJECT" ${AGENTS:+--agents "$AGENTS"} > ".gemini/state/adopt-scan.tsv"
```
...
`bash "$HOME/.gemini/scripts/lib/project-gates.sh" detect` and never `run`. And when a scanned file's claim bears on the plan
...
- **delta** — `bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" delta <kind> <project-path> <baseline-path>`. Do **not** substitute
...
- **prescribed** — `bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" prescribed <kind> <name>` (exit 0 = yes).
...
while IFS="$TABC" read -r kind rel name agent; do
  ...
  if [ "$agent" = "-" ]; then
    src="$(awk -F'\t' -v k="$kind" -v n="$name" '$1==k && $2==n {print $4; exit}' \
             ".gemini/state/adopt-shipped.tsv")"
  else
    src="$(awk -F'\t' -v k="$kind" -v n="$name" -v g="$agent" \
             '$1==k && $2==n && $3==g {print $4; exit}' ".gemini/state/adopt-shipped.tsv")"
  fi
  coll=no;  [ -n "$src" ] && coll=yes
  presc=no; bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" prescribed "$kind" "$name" >/dev/null 2>&1 && presc=yes
  delta=unknown
  [ -n "$src" ] && delta="$(bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" delta "$kind" "$PROJECT/$rel" "$src")"
  ...
  line="$(bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" classify "$kind" "$coll" "$delta" "$presc")" || exit 1
  printf '%s\t%s\t%s\t%s\n' "${line%%"$TABC"*}" "$kind" "$rel" "${line#*"$TABC"}"
done < ".gemini/state/adopt-scan.tsv" > ".gemini/state/adopt-classified.tsv"
```
...
```bash
# ADB-SNIPPET: propose
bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" roles-infer "$PROJECT" > ".gemini/state/adopt-roles.tsv"
bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" propose < ".gemini/state/adopt-roles.tsv" > ".gemini/state/adopt-agents.toml"
```
...
Detected gates belong in the same file. `bash "$HOME/.gemini/scripts/lib/project-gates.sh" detect` reports what the ecosystem
...
```bash
# ADB-SNIPPET: hygiene
bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" hygiene "$PROJECT" ${AGENTS:+--agents "$AGENTS"} > ".gemini/state/adopt-hygiene.tsv"
```
...
```bash
# ADB-SNIPPET: pin
bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" pin-render \
  "$(git -C "$BASELINE" describe --tags --abbrev=0 2>/dev/null || echo '')" \
  "$(git -C "$BASELINE" rev-parse HEAD)" \
  "$(date -u +%Y-%m-%d)" \
  "$(bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" stack "$PROJECT")" \
  "${AGENTS:-claude,codex,gemini}" > ".gemini/state/adopt-upstream.toml"
```
...
```bash
# ADB-SNIPPET: drift
bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" pin-drift .ai-dev-baseline/upstream.toml "$BASELINE"
```
...
```bash
# ADB-SNIPPET: plan
bash "$HOME/.gemini/scripts/lib/adopt-lib.sh" plan < ".gemini/state/adopt-classified.tsv"
```
...
Without `--apply`, everything above is a report and nothing is written outside `.gemini/state`.
...
adopt_create ".gemini/state/adopt-agents.toml" "$PROJECT/agents.toml" "agents.toml"
...
if [ -L "$PROJECT/.ai-dev-baseline" ]; then
  ...
  adopt_create ".gemini/state/adopt-upstream.toml" "$PROJECT/.ai-dev-baseline/upstream.toml" "the upstream pin"
fi
```
...
( cd "$PROJECT" || exit 1
  bash "$HOME/.gemini/scripts/lib/repo-settings.sh" status      # show what would change BEFORE changing it
  bash "$HOME/.gemini/scripts/lib/repo-settings.sh" apply )     # required status checks, then allow_auto_merge
```
