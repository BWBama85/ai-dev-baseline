...
- **Flaky-CI gambling.** Never re-run a red job to get a lucky green
  ...
  (`bash "$HOME/.codex/scripts/lib/ci-health.sh" classify --run <id>` → **23**) is a fact about the provider:
...
(`bash "$HOME/.codex/scripts/lib/project-gates.sh" run`). If the fix is more than a
