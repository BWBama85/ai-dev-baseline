...
`bash "$HOME/.codex/scripts/lib/implement-lib.sh"` (`admit` / `release` / the step subcommands) is the mechanism, and
...
than accommodated. Preflight asks `bash "$HOME/.codex/scripts/lib/implement-lib.sh" admit .codex/state` before anything is
...
Stage every marker write inside `.codex/state/` (`.marker.tmp` → `mv`) so the rename is atomic.
...
**inline** with the literal token (`bash "$HOME/.codex/scripts/lib/implement-lib.sh" release --token <token> .codex/state`) —
...
line, the claim carries it: `jq -r .token .codex/state/gap-analysis.lock`.
...
`.codex/state` is repo-relative and per-agent, which is the boundary `admit` already enforces; a
