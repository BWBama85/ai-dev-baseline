...
`bash "$HOME/.codex/scripts/lib/role-dispatch.sh" invoke` runs the resolved agent's CLI under a **45-minute (2700 s) hang
...
- **A claim survives a killed run** → its lease (9000s) expires and the next run breaks it with a
  NOTE. Waiting is correct; deleting `.codex/state/gap-analysis.lock` is the documented escape
  when you are certain the run is dead.
- **A refusal that does NOT clear itself** → an abandoned marker whose branch ref survives, a
  malformed marker, a persistent `gh` failure, wrong permissions on `.codex/state`. Each refusal
...
- **The Stop hook nags about a run you never started** → it is reading a marker owned by another
  ...
  `jq -r '.owner, .branch' .codex/state/implement-issue-active.json`: an `owner` that is not
