...
mkdir -p .codex/state
rm -f .codex/state/implement-issue-active.json .codex/state/implement-issue-blocked.json
...
rm -f .codex/state/gap-prompt.txt .codex/state/gaps.md .codex/state/gaps.err \
...
# `.codex/state/review-*.md` to the `rm -f` above looks equivalent and is not — an unmatched glob
...
find .codex/state -maxdepth 1 -type f \( -name 'review-*.md' -o -name 'review-*.err' \) \
     -exec rm -f {} +
```
...
- **On any path that STOPS the run here** — a BLOCKING finding you surface, or a gap-analysis
  ...
  ```bash
  rm -f .codex/state/gap-analysis.lock
  ```
