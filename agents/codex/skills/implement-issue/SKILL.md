...
bash "$HOME/.codex/scripts/lib/implement-lib.sh" admit .codex/state; ADMIT=$?
case "$ADMIT" in
...
for n in "${ISSUE_NUMS[@]}"; do
  gh issue view "$n" --json number,title,body,labels,author,comments,milestone,state > "/tmp/issue-$n.json" \
    || { bash "$HOME/.codex/scripts/lib/implement-lib.sh" release .codex/state; echo "ERROR: issue #$n not found in this repo — verify repo scope"; exit 1; }
done
...
for n in "${ISSUE_NUMS[@]}"; do
  gh api "repos/{owner}/{repo}/issues/$n" --jq '.author_association' > "/tmp/issue-$n.assoc" \
    || { bash "$HOME/.codex/scripts/lib/implement-lib.sh" release .codex/state; echo "ERROR: could not read #$n's author association"; exit 1; }
done
```
...
for n in "${ISSUE_NUMS[@]}"; do
  ...
  [ "$st" = "OPEN" ] || { bash "$HOME/.codex/scripts/lib/implement-lib.sh" release .codex/state
...
for n in "${ISSUE_NUMS[@]}"; do
  ...
  TEXT="$(jq -r --arg assoc "$(cat "/tmp/issue-$n.assoc")" '
      ...
      | join("\n\n---\n\n")' "/tmp/issue-$n.json")" \
    || { bash "$HOME/.codex/scripts/lib/implement-lib.sh" release .codex/state; echo "ERROR: could not read issue #$n's text"; exit 1; }
  printf '%s' "$TEXT" | bash "$HOME/.codex/scripts/lib/role-dispatch.sh" untrusted "github-issue #$n" >> .codex/state/gap-prompt.txt \
    || { bash "$HOME/.codex/scripts/lib/implement-lib.sh" release .codex/state; echo "ERROR: could not contain issue #$n's text — do NOT fall back to pasting it raw"; exit 1; }
done
...
- **On any path that STOPS the run here** — a BLOCKING finding you surface, or a gap-analysis
  ...
  ```bash
  bash "$HOME/.codex/scripts/lib/implement-lib.sh" release .codex/state
  ```
