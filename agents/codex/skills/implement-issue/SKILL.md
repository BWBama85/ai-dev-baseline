...
so a second run is **refused**, not accommodated. Preflight asks `bash "$HOME/.codex/scripts/lib/implement-lib.sh" admit` before it
...
different `.codex/state`s and never collide on these paths at all; they still share HEAD, and the
...
```bash
stop_run() { bash "$HOME/.codex/scripts/lib/implement-lib.sh" release .codex/state; echo "$1"; exit 1; }
...
jq -n --arg branch "$BRANCH" --arg issue "$ISSUE_CSV" \
   ...
   > .codex/state/.marker.tmp && mv .codex/state/.marker.tmp .codex/state/implement-issue-active.json
...
- **A claim survives a killed run** → its lease (twice the dispatch backstop plus an hour) expires
  ...
  you are certain the run is dead, deleting `.codex/state/gap-analysis.lock` is the documented
  escape.
