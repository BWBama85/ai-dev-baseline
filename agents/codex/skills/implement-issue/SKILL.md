...
bash "$HOME/.codex/scripts/lib/docs-lib.sh" probe-record --server <name> --result usable|degraded|absent \
  --evidence '<what you observed — the call and its result, not "it worked">'

bash "$HOME/.codex/scripts/lib/docs-lib.sh" verdict; VRC=$?
case "$VRC" in
...
```bash
bash "$HOME/.codex/scripts/lib/docs-lib.sh" consulted --surface '<the API/service/flag>' --rung <1|2|3> \
...
```bash
bash "$HOME/.codex/scripts/lib/docs-lib.sh" none-needed --justification '<why every surface here is trivial or already-proven>'
```
...
`bash "$HOME/.codex/scripts/lib/docs-lib.sh" report` returns **11** for a run that recorded neither kind. Step 10 and step 11 both
...
```bash
bash "$HOME/.codex/scripts/lib/docs-lib.sh" report; DRC=$?
case "$DRC" in
...
**State the documentation disposition, in one line (#422).** Render it with `bash "$HOME/.codex/scripts/lib/docs-lib.sh" report`
