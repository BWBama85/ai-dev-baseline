...
- `codex` → `codex exec --cd <repo> -`; `gemini` → `agy -p`. A cross-agent
  `codex exec` needs a **≥7-minute** timeout. `bash "$HOME/.codex/scripts/lib/role-dispatch.sh" invoke` wraps both
...
```bash
printf '%s' "$GAP_PROMPT" | bash "$HOME/.codex/scripts/lib/role-dispatch.sh" invoke gap_analysis > .codex/state/gaps.md
```
