...
| **2 — deferred to the PR layer** | no in-session slot is usable, **but** `bash "$HOME/.codex/scripts/lib/role-dispatch.sh" bots --declared` lists an async reviewer | mark the slot **deferred**, say so, and proceed to the PR. |
