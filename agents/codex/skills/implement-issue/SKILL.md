...
collide — so a second run is **refused**, not accommodated. Preflight asks `bash "$HOME/.codex/scripts/lib/implement-lib.sh" admit`
...
**"Per agent" is not a footnote.** The scope is this agent's `.codex/state`, so what is enforced is
...
Always stage marker writes inside `.codex/state/` (`.marker.tmp` → `mv`) so the
...
shelling for you — `bash "$HOME/.codex/scripts/lib/role-dispatch.sh" resolve <role>` prints the token(s);
`bash "$HOME/.codex/scripts/lib/role-dispatch.sh" invoke <role|agent>` (prompt on stdin) runs one agent's CLI and
...
- `codex` → `codex exec --cd <repo> -`; `gemini` → `agy -p`. `bash "$HOME/.codex/scripts/lib/role-dispatch.sh" invoke`
...
`jq -r .token .codex/state/gap-analysis.lock`.
...
**THE SNAPSHOT LANDS IN `.codex/state`, NEVER IN `/tmp` (#250).** These two files hold the
...
any local account can reach. `.codex/state` is repo-relative
...
**UNTRUSTED READ SITE — `.codex/state/issue-<n>.json`.** Its `body` and every `comments[].body`
...
`bash "$HOME/.codex/scripts/lib/state-assert.sh" observe pr <N>` (`base/practices/verify-before-asserting.md`).
...
- **A refusal that does NOT clear itself** → some do not, and the message says which. An abandoned
  ...
  on `.codex/state`. Each refusal prints the recovery: finish the branch, run `/cleanup`, or delete
...
- **Stop hook nags about a run you never started** → it is reading a marker owned by
  ...
  `jq -r '.owner, .branch' .codex/state/implement-issue-active.json`: an `owner` that
