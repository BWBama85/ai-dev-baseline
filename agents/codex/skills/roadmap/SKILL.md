  ...
  **Resolve it before emitting it, and never invent one (#188).** An unresolvable slash command
  ...
  case "$CMD" in
    "$PFX"?*)
      ...
      # its SKILL.md sits right there. codex debug prompt-input is empty for agents with no such
      ...
      if [ -n "$SKILL_NAME" ] && [ "$RESOLVES" -eq 0 ] && [ -z "$PROBE" ]; then
        ...
        # and reports a present skill as missing. .codex/skills .agents/skills is a LIST (Codex loads both
