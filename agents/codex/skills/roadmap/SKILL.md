...
4. **The branch must be green (#78).** A drained checklist says the *requirements* are done; it
   ...
   - **not-green** → withhold the cut and name the failing check. Normally a `/debug` signal —
     **but classify the check before calling it one (#300, D58).** A job that never acquired a
     ...
     bash "$HOME/.codex/scripts/lib/ci-health.sh" classify --run <id>   # 23 = nothing that failed executed a step
     ```
...
- **Met** (armed, predicate satisfied, no unacknowledged canceled blocker, **and the branch is
  ...
  **Resolve it before emitting it, and never invent one (#188).** An unresolvable slash command
  ...
  CMDS="$(printf '%s' "$ARTIFACT_BODY" | bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" release-command)" || { echo "ERROR: release-command extraction failed"; exit 1; }
  ...
  case "$CMD" in
    "$PFX"?*)
      ...
      # codex debug prompt-input is empty for agents with no such command; the frontmatter contract
      ...
      if [ -n "$SKILL_NAME" ] && [ "$RESOLVES" -eq 0 ] && [ -z "$PROBE" ]; then
        ...
        # .codex/skills .agents/skills is a LIST (Codex loads both `.codex/skills` and `.agents/skills`) and is
        # EMPTY where project-local discovery is unestablished, in which case only the user root is
        # searched.
        for sub in .codex/skills .agents/skills; do
        ...
        if [ "$RESOLVES" -eq 0 ]; then
          for d in "${CODEX_HOME:-$HOME/.codex}/skills"; do
...
- **Re-derive every dependency edge from its source — never carry one forward.** The
  ...
  DEPS="$(printf '%s' "$BODY" | bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" deps-from-body "$N")" \
...
2. **Check the recorded set before surfacing anything.** A question whose id already appears in
   ...
   ```bash
   ANSWERED="$(printf '%s' "$ARTIFACT_BODY" | bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" decisions)" \
...
1. **Dependencies first.** Never place a bundle before a bundle it depends on. The edge set is
   the one step 4 **re-derived this run** via `bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" deps-from-body` — explicit
...
while IFS= read -r row; do
  ...
  DDEPS="$(printf '%s' "$DCELL" | bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" deps-from-body "$Q")" \
...
printf '%s' "$UNIV_JSON" \
  ...
  | bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" compose-candidates "$ROADMAP_NUM" bug > "$CDIR/cand.tsv" \
...
bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" compose-select < "$CDIR/cand.tsv" > "$CDIR/plan" \
...
OPEN_JSON="$(gh api --paginate "repos/$REPO/issues?state=open&per_page=100")" \
  || { echo "ERROR: could not list open issues — hard stop"; exit 1; }
OPEN_NUMS="$(printf '%s' "$OPEN_JSON" | bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" open-issues)" \
  || { echo "ERROR: could not parse the open-issue read — hard stop"; exit 1; }
PR_LIMIT=1000
...
case "$(bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" read-complete "$GOT" "$EXPECTED")" in
  complete) : ;;
...
if [ -n "$LABEL" ]; then
  ...
  bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" slug-ok "$REPO" || exit 1
...
MEMBERS="${MEMBERS:-}"
VERDICT="$(printf '%s\n' "$MEMBERS" | cut -f2 | bash "$HOME/.codex/scripts/lib/roadmap-lib.sh" emit-verdict)" \
