  ...
  **Resolve it before emitting it, and never invent one (#188).** An unresolvable slash command
  ...
  case "$CMD" in
    "$PFX"?*)
      ...
      # naive string compare. codex debug prompt-input is empty for agents with no such command.
      ...
      if [ -n "$PROBE" ] && command -v "${PROBE%% *}" >/dev/null 2>&1; then
        ...
        # .codex/skills .agents/skills is a LIST (Codex loads both `.codex/skills` and `.agents/skills`) and
        ...
        SEARCH=""
        for sub in .codex/skills .agents/skills; do SEARCH="$SEARCH $GITROOT/$sub"; done
        for d in $SEARCH "${CODEX_HOME:-$HOME/.codex}/skills"; do
