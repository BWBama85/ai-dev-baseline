...
- **So: a third field on the report record**, `<category>TAB<item>TAB<proof>`, rendered by
  `bash "$HOME/.codex/scripts/lib/cleanup-lib.sh" report`. A record with no third field renders exactly as it did before, so
...
- **Never a `#N` beside a status word.** `bash "$HOME/.codex/scripts/lib/state-assert.sh" lint` rejects that shape unless it is
...
if ! SCAN="$(bash "$HOME/.codex/scripts/lib/cleanup-lib.sh" state-scan --with-identity "$STATE")"; then
...
SV="$(bash "$HOME/.codex/scripts/lib/cleanup-lib.sh" state-verdict survey "$LOCK" "$RUN_NOW")" || SV=keep
...
LV="$(bash "$HOME/.codex/scripts/lib/cleanup-lib.sh" state-verdict rules "$RUN_NOW")" || LV=keep
...
while IFS="$TABC" read -r kind sfile key ident; do
  case "$kind" in
    ...
    threads)
      ...
      PRST="$(pr_state "$key")"
      TV="$(bash "$HOME/.codex/scripts/lib/cleanup-lib.sh" state-verdict threads "$PRST")" || continue
      [ "$TV" = stale ] || continue
    ...
    sweep)
      ...
      PRST="$(pr_state "$key")"
      WV="$(bash "$HOME/.codex/scripts/lib/cleanup-lib.sh" state-verdict sweep "$PRST")" || continue
      [ "$WV" = stale ] || continue
      sweep_file "$sfile" "$ident" "PR $PRST"
      ;;
  esac
...
while IFS="$TABC" read -r kind sfile key ident; do
  ...
  # A bounded filename open (bash "$HOME/.codex/scripts/lib/cleanup-lib.sh" file-size), never a parent-shell redirect: these
  ...
  sz="$(bash "$HOME/.codex/scripts/lib/cleanup-lib.sh" file-size "$sfile" 2>/dev/null)" || sz=""
