...
if [ "$WATCH" = "1" ]; then
  ...
  # bounds calls the library directly (bash "$HOME/.codex/scripts/lib/pr-watch.sh" wait --pr N --interval S --max-secs S)
  # and then runs this skill without --watch.
  bash "$HOME/.codex/scripts/lib/pr-watch.sh" wait --pr "$PR_NUM"; WRC=$?
else
