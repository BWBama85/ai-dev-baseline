...
# Reported by the declared reviewer on PR #429.
bash "$HOME/.codex/scripts/lib/pattern-ledger.sh" reclaim; LRC=$?
case "$LRC" in
  0)  : ;;
...
```bash
bash "$HOME/.codex/scripts/lib/implement-lib.sh" dispatch-review --criteria-from-pr "$PR_NUM" .codex/state <token>
```
...
sweep_once() {
  case "$RUNG" in
    independent*|same-model*)
      bash "$HOME/.codex/scripts/lib/implement-lib.sh" dispatch-sweep ${EFFORT:+--effort "$EFFORT"} --pr "$PR_NUM" --findings "$FINDINGS" .codex/state "$REVIEW_TOKEN" ;;
    deferred*|none*)
      bash "$HOME/.codex/scripts/lib/implement-lib.sh" dispatch-sweep --skipped --pr "$PR_NUM" --findings "$FINDINGS" .codex/state ;;
...
bash "$HOME/.codex/scripts/lib/pattern-ledger.sh" record --class <slug> … ; RRC=$?
case "$RRC" in
...
bash "$HOME/.codex/scripts/lib/pattern-ledger.sh" promote --class <slug> --rule '<the sweep to run before the next PR>'; PRC=$?
case "$PRC" in
...
STATS_AFTER="$(bash "$HOME/.codex/scripts/lib/pattern-ledger.sh" stats --pr "$PR_NUM")"; SRC=$?
case "$SRC" in
...
CLASSES_AFTER="$(bash "$HOME/.codex/scripts/lib/pattern-ledger.sh" classes)"
