...
| `scripts/lib/pinned-install.sh` | The **second install model** (#285, D77) — one released version vendored into one project tree, beside the global symlink install rather than replacing it. It always installs from a **published release artifact**, checksummed against that artifact's own `SHA256SUMS` record before anything is unpacked, even when driven from a clone; the payload lands under `.<agent>/adb/` (never `.claude/scripts/`, which is `handling-the-unknown.md`'s home for a project's OWN gate policy) and the vendored skills are **re-anchored** off `$HOME/.<agent>/scripts/lib/` so two pinned projects and the global install cannot reach into each other. The pin gains `mode = "pinned"` — presence of `upstream.toml` is NOT the discriminator, since `/adopt` writes that file for global-mode projects too — and a **receipt** of per-file digests is what `status` re-checks and what uninstall removes by, so a file you edited is kept and named rather than deleted. Gemini is refused: it has no project-local skill discovery. Regression-tested by `scripts/check-pinned-install.sh`; reachable as `baseline pinned`, `install.sh --pinned`, `uninstall.sh --pinned` |
