...
3. **Run `scripts/selfcheck.sh` before every push.** It mirrors every *offline* check CI runs —
   ...
   **CI runs four steps fewer on the macOS leg** (#339; the second since PR #429, the third since
   ...
   and the two pushes after it were cancelled by that job's 45-minute ceiling with the step still
   running. On run 33043334817 (2026-08-27), the session-context harness took 1370s in
...
| `agents/claude/settings.fragment.json` · `scripts/check-settings-fragment.sh` | The installer's **second settings surface** (#248, D95-D98) — the answer to *what may the installer write into `~/.claude/settings.json` that is not a hook?* `wire_hooks` nests everything it is given beneath `.hooks`, so a `sandbox` block added to the hook payload lands at `.hooks.sandbox` and means nothing; there was no non-hook surface at all, which is what #288 and epic #283 waited on. **It applies WHOLE or not at all** (D100, owner decision after six review rounds): the fragment installs only when every key it ships is absent, and anything already there refuses the lot and is named. D95's per-leaf boundary — write what you can, skip what they own, remember what they deleted — is what rounds 3 to 6 kept finding defects IN rather than in the policy it carried, and the states are gone with it. An established install updates only while every recorded leaf still carries the value recorded for it; any divergence means the operator has taken the surface over, so nothing is written. That is also why no tombstone is needed: a deleted leaf is not remembered as ours, so a later re-add by hand is never deleted on the strength of a record. An adopter's `sandbox.excludedCommands` is a sibling of our `sandbox.enabled` and is never in the way. A leaf the receipt records that the payload no longer ships is **pruned** — the settings analogue of `adb_agent_manifest_retired`, without which a dropped key sits in every adopter's file with no owner. **Currency is a DIGEST question**: the receipt records the digest of the payload it applied, because owned leaf PATHS answer it wrongly in both directions — a leaf the operator already owned is never recorded (so a path-set comparison reports pending forever and re-runs the installer every session) and a changed shipped VALUE leaves the path set identical (so it never notices a payload a `git pull` just changed). The receipt also records the **containers** the install created, because an adopter who already had `{"sandbox":{}}` must keep it and ownership of an object cannot be derived from the leaf paths at removal time. Every disposition that can carry rows OWNS them, transient skips included; what guards a doctored receipt is not the disposition but the per-row validation — a **non-empty** array of string components, since `all(.[]; …)` is vacuously true for `[]` and `delpaths([[]])` nulls the whole settings document — plus the rule that a leaf is removed only while its live value still equals the recorded one. That guards against ACCIDENTAL damage; a receipt someone deliberately hand-edited is out of scope (D102). So is a read that fails while the reads around it succeed — a transient failure — whereas a persistent one (unreadable, malformed, missing `jq`) stays in scope (D106). **User scope only** (D97) — and the reason recorded there is NOT the owner's stated premise, which the implementation re-resolved and found false for the shipped set: `deny` entries merge from every scope, so these keys *would* be honoured project-wide. What is genuinely user/managed-only is `filesystem.disabled`, `strictAllowlist` and the `mask` family, none of which ship. It stays user-scoped because the global model is user-level config and because a pinned install's `.claude/settings.json` is TRACKED, so `pinned-install.sh` refuses them and says so. The receipt also carries a **disposition**, because "no `sandbox` keys" arises four ways and only two are choices (D98) — a below-floor or unprobeable skip is RETRIED, and `bin/baseline`'s `adb_settings_pending` is the third question the `current` path now asks, since links-OK + hooks-wired used to exit before ever looking (the #242 shape one surface over). The shipped values are D96's, and two keys are deliberately absent: `filesystem.disabled` turns isolation *off*, and `strictAllowlist` was never in the decided key set. Regression-tested by `scripts/check-settings-fragment.sh`, whose mutation rows are each required red on their own witness, running only the block that holds it plus that block's dependencies (#468, D103) |
