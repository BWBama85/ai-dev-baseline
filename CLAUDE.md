...
3. **Run `scripts/selfcheck.sh` before every push.** It mirrors every *offline* check CI runs
   ...
   **One CI step is deliberately not mirrored** (#122, recorded as D13 in
   ...
   repo's **live** branch protection to catch a newly added CI job that stayed non-required. A
   live read has no honest offline behavior — it would be red on every offline run, or green when
   it could not read, and a fail-open gate is worse than none. `selfcheck` keeps the offline half
   (`check-repo-settings.sh` drives the predicate through a `gh` stub), so the only thing a local
   green cannot predict is the one check whose input is external mutable state.
