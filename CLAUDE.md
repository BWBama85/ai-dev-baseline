...
4. **Shell code targets bash 5.3 and must be shellcheck-clean.** The runtime floor is
   ...
   **Two carve-outs, both load-bearing:**
   ...
   parse and a bootstrap probe under the **oldest sub-floor interpreter** available, with
   a stated SKIP where none exists. It rides the bare invocation, so `selfcheck-macos` —
   the one per-PR job with a real 3.2.57 — is what actually exercises it. It proves
   parseability and gate reachability, **not** that every function behaves on 3.2; a
   `mapfile` or `declare -A` inside a function body is still review's job.
