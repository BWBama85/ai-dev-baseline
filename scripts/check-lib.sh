...
#      check_make_stub_repo + check_write_stub: the throwaway identity wrapper, the local+bare-origin
#      scaffold, the "repo with an origin URL" scaffold and the executable-stub writer,
#      accounting-neutral so either family can guard them.
#   4. PR reviewer-signal payload builders (§ further down) — check_pr_reviews_json and friends,
#      plus check_pr_json / check_declare_bots: the GitHub response SHAPES both PR-guard suites stub.
#   5. mutation-harness family (§ with check_mutate_line) — check_mutate_literal / check_mut /
#      check_mutation_pool: inject one defect per row into a COPY of the tree and require the suite
#      under test to come back RED, on its own witness AND with a failing exit status. Uses family
#      2's counters (ok / bad), so a suite that drives it must also end with check_summary.
#
...
# The SECOND mutation contract, and a deliberate SIBLING of `check_mutate_line` above rather than a
# replacement for it. That one requires whole-line uniqueness and applies a `sed` script; this one
# replaces a LITERAL SUBSTRING by index. Both belong: a substring edit cannot demand whole-line
# uniqueness (the needles here are routinely fragments of a longer line), and a sed script cannot
# carry an arbitrary literal — the first version of the adopt table was written in sed and EIGHT OF
# TWENTY rows silently matched nothing, every one of them because of a character special to sed, to
# the shell, or to the heredoc it was written in. A mutation that matches nothing is a test that
# proves nothing.
#
# Two suites drove byte-identical copies of the apply/verify/score machinery below —
# check-adopt.sh and check-adopt-readiness.sh — and one copy had DIVERGED in the direction that
# matters (see check_mutation_pool's own header). A shared home is what stops the next divergence
# being invisible. check-common-lib.sh keeps its own harness deliberately: its rows are `sed`
# scripts, not literals, so it is a `check_mutate_line` caller and not one of these.

# check_mutate_literal <file> <old> <new> — replace the FIRST occurrence of the literal <old> with
# <new>, IN PLACE, and prove the edit landed. Returns:
#   0  the mutation applied
#   1  the rewrite itself failed (unreadable file, unwritable dir)
#   2  the literal matched NOTHING, so the file is unchanged — this row tests nothing
#
# CODE 2 IS THE POINT. A literal that matches nothing produces an unchanged copy, the suite under
# test passes, and a harness without this check would blame the guard for a mutation that never
# happened — the same silence the harness exists to detect, one level up. Ten of the adopt table's
# first twenty rows failed exactly here, across three separate causes.
#
# THE STRINGS ARRIVE THROUGH `ENVIRON`, NOT `awk -v`, and that is load-bearing. `-v x=…` processes
# backslash escape sequences in the assignment: a literal containing `\$` reaches the program as
# `$`, and one containing `\n` arrives as a REAL NEWLINE that can never match inside a single line.
# The two adopt rows whose source text contains a backslash were the exact two that kept reporting
# "did not apply" against literals generated byte-exactly from the source file. `ENVIRON` performs
# no such processing, so the bytes arrive as written.
#
# `index()` sees ONE RECORD AT A TIME, so a literal spanning two source lines can never match. That
# is a constraint on the table, not a defect here — and code 2 is what reports it.
#
# Writes a sibling and renames over the target rather than using `sed -i`, whose spelling differs
# between BSD and GNU, and so that a failed rewrite cannot leave the target half-written. The
# sibling's name is FIXED rather than `mktemp`'d because each pool worker owns its own tree copy, so
# two workers can never name the same path — if that ever stops being true, this needs a temp name.
...
# The table. `check_mut <name> <old-literal> <new-literal> <witness>` — FIXED STRINGS, never regexes.
#
# THE WITNESS IS THE ASSERTION LABEL (or a distinctive fragment of one), because a mutation that
# goes red for the WRONG reason is not evidence. That is the `fires:<witness>` contract #213
# established, applied to a whole suite instead of one lint rule.
...
# _check_mut_witness <output> <witness> — did the child fail on THIS mutation's witness? True when
# some line of <output> begins with `FAIL: ` AND carries <witness>.
#
# MATCHED WITH `case` OVER A HERE-STRING, NOT `printf | grep -q`, and that is a correctness fix
# inherited from #324 rather than a style preference. These suites set `pipefail`. `grep -q` exits
# the instant it matches, closing the pipe while `printf` still has the rest of a large output
# queued — `printf` then dies of SIGPIPE and pipefail promotes that to the pipeline's status. So the
# witness check returned NON-ZERO for exactly the outputs where the witness was found EARLY, and the
# harness reported "went red, but NOT on its witness" about a witness sitting on line 1. Measured: a
# 348906-byte buffer with the witness first gives rc=141 under pipefail. No pipe, no signal, nothing
# for pipefail to promote.
#
# THE WITNESS IS INTERPOLATED QUOTED (`*"$2"*`), so it is matched LITERALLY — quoting removes the
# glob meaning of `*`, `?` and `[…]` inside a `case` pattern. That is not a detail: three live
# witnesses carry one (`a blanket *.json IS reported`, and two naming `[gates.state]` /`[roles]`),
# and unquoted they would either widen to a wildcard or, for the bracket pair, stop matching their
# own FAIL line altogether. Verified rather than assumed.
#
# PER LINE, not over the whole blob, which is a hair STRICTER than the anchored
# `*"FAIL: $witness"*` the readiness copy used: a witness that is a mid-label fragment (the adopt
# table's form) now has to appear on a FAIL line rather than anywhere in several hundred KB of
# output, where a passing assertion's own echo could have carried it.
_check_mut_witness() {
  local line
...
# One mutation, start to verdict. Writes a single `<verdict>|<why>` line to `<workdir>/mut-<i>/verdict`.
#
# IT RETURNS ITS VERDICT THROUGH A FILE, not through `ok`/`bad`, because it runs in a BACKGROUND
# SUBSHELL and a counter incremented there dies with it. That is the same fiction #259 records, and
# the fix is the same: the parent does the counting, over results the child could not have discarded.
# Its own exit status stays 0 whatever it decided — the pool reaps exactly one job per `wait -n`, and
# a non-zero worker is what used to send it down a fallback that drained every child at once.
_check_mut_one() {   # <index> <workdir> <prepare-fn> <run-fn>
  ...
  # THE STATUS AND THE WITNESS, BOTH. Matching the witness STRING while throwing the child's exit
  # code away makes "N mutations proven RED" a claim about what the child PRINTED, never about
  # whether it FAILED: a child that prints the expected line and then exits 0 — a stray `exit 0`, a
  # verdict-line refactor, a `set -u` abort after the diagnostic — is recorded as a proof the
  # harness does not have. Require EXACTLY 1: that is what a failed ASSERTION exits, while a crash,
  # a `set -u` abort or a usage error exit something else, and those are the suite dying rather than
  # the guard firing (base/practices/self-review.md).
...
# check_mutation_pool <label> <workdir> <prepare-fn> <run-fn> <pool-cap> — run every row of the
# table through a bounded worker pool and score the results into family 2's counters.
#
#   <prepare-fn> <copy-dir>  build the tree copy at <copy-dir> and PRINT the absolute path of the
#                            file to mutate. Non-zero (or empty stdout) fails that row. It must
#                            print NOTHING ELSE — its stdout is the target path.
#   <run-fn>     <copy-dir>  run the suite under test inside <copy-dir>. Its stdout+stderr are
#                            captured by the pool; its EXIT STATUS is half the verdict.
#
# A BOUNDED POOL, for the same reason `scripts/selfcheck.sh` runs its registry through one: each row
# costs a full suite run, and the adopt table's twenty rows measured 658s serially. The work is
# embarrassingly parallel and each row owns its own tree copy, so nothing is shared but the CPU.
# The WIDTH IS ASKED FOR, never hardcoded: `adb_pool_size` is the one home for min(cpu, cap)
# (scripts/lib/common.sh, #335), and the three open-coded probe chains it replaced had three
# different answers. The cap stays a parameter because the two adopt suites deliberately differ
# (D66 records why readiness caps at 4).
#
# THE PARENT SCORES, over every index — so a worker that died without writing a verdict is a FAILURE
# rather than a silently missing row, and the row count is asserted against the table so a loop that
# iterated zero times is loud instead of indistinguishable from "scored everything and all passed".
check_mutation_pool() {
  ...
  for (( i = 0; i < n; i++ )); do
    ...
    # 8-worker fixture at pool 4: 5s against the correct 2s. `_check_mut_one` returns 0
    # unconditionally, so the fallback protected nothing the contract does not already give.
  ...
  for (( i = 0; i < n; i++ )); do
    ...
    if [ "$verdict" = ok ]; then
      ...
      # nothing at all, which is a different and worse failure than a guard that missed one.
      case "$why" in
...
# check_make_stub_repo <dir> <origin-url> — the OTHER throwaway-repo shape, and the sibling of
# check_make_repo_pair rather than a variant of it: there is no bare repo to push to, only a repo
# whose `origin` names a URL. Creates <dir>, inits it, stamps the throwaway identity + signing-off
# config, and points `origin` at <origin-url>. Returns non-zero WITHOUT exiting so a `set -u` caller
# can guard it.
#
# THE REMOTE IS LOAD-BEARING, not set dressing (#173): the code under test anchors every `gh` read
# to the checkout's git origin, because a bare `--pr 7` names no repository and the documented
# `GH_REPO` variable silently redirects gh's `repos/{owner}/{repo}` expansion. The origin slug must
# agree with whatever the PR fixture reports as `base.repo.full_name`, or every scenario fails
# closed. Three suites hand-rolled this (check-pr-review.sh, check-pr-watch.sh, check-claims-guard.sh)
# and one of the three omitted the identity config, which is exactly how a contributor whose global
# config sets `commit.gpgsign=true` gets a fixture that cannot commit.
...
# check_write_stub <path> — read a stub program from STDIN, write it to <path> (creating its parent)
# and make it executable. The ONE home for the `cat > "$BIN/gh" <<'STUB' … STUB; chmod +x` move that
# eight sites open-coded across four suites, where forgetting the `chmod` produces a PATH shim that
# is silently never used — the suite then exercises the REAL `gh`, or nothing, and passes for the
# wrong reason. Returns non-zero WITHOUT exiting.
#
# Usage:  check_write_stub "$SBIN/gh" <<'STUB'
#         #!/usr/bin/env bash
#         …
#         STUB
check_write_stub() {
  # `${1%/*}` is the WHOLE path when it carries no slash, so a bare `stub` name would `mkdir` the
  # file it is about to write. Only create a parent when there demonstrably is one.
...
#               [--head-ref X] — the PULL-REQUEST OBJECT itself, the fifth shape both suites stub.
#
# NAMED FLAGS, AND THAT IS THE WHOLE REASON THIS FUNCTION LOOKS LIKE THIS (#373). The two suites'
# `pr_fx` had DIFFERENT positional signatures for the same fixture — pr-watch inserted `state` and
# `merged_at` at positions 2 and 3, so pr-review's `pr_fx "$sha" "acme/widget" ""` and pr-watch's
# `pr_fx "$sha" "open" ""` mean different things in the same three words. A unified positional
# SUPERSET is the obvious merge and it is the dangerous one: every existing call would shift its
# arguments one or two places to the right and land on a VALID-but-wrong fixture — a base slug read
# as a state, an empty head-slug read as an empty merged_at — which produces no error anywhere, just
# scenarios quietly testing something else. Flags cannot mis-shift.
#
# LAST FLAG WINS, which is what lets each suite keep a one-line wrapper carrying its OWN constants
# (`HEAD_SHA`, `HEAD_REF`, the origin slug) and still let a scenario override one field:
#   pr_fx() { check_pr_json "$S/pr.json" --sha "$HEAD_SHA" --head-ref "$HEAD_REF" \
#               --base-slug acme/widget --head-slug acme/widget "$@"; }
#   pr_fx --head-slug ""            # a deleted fork
# This file deliberately carries NO fixture constants: they live where the assertions that read them
# live. Every field therefore defaults to empty, and an empty value is meaningful for two of them —
# an empty `--head-slug` renders `head.repo` as JSON `null` (the deleted-fork shape the staleness
# anchor must degrade on), and an empty `--merged-at` renders `merged_at` as `null`.
#
# An unknown flag, or a flag with no value, records a FAILURE rather than being skipped: a silently
# dropped field is a fixture that tests something other than what its call site says it does.
check_pr_json() {
  local out="$1"; shift
...
# The TRI-STATE is what both suites are pinning (#134), so the two spellings must not drift: `[]`
# (no reviewer declared -> arm) and NO FILE AT ALL (unknowable -> fail closed) are different
# answers, and their collapse into each other IS the defect.
...
# because the manifest resolves project-first then user-level, and leaving the user-level copy
# behind makes "undeclared" quietly mean "declared somewhere else".
