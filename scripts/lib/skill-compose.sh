...
# The compose engine (awk). Reads the overrides file FIRST, then the base skill; emits the
# composed skill on stdout. Exit: 0 ok · 2 overrides error · 3 base/anchor error. Kept to
# POSIX awk (index/substr/2-arg match/gsub — no gensub, no match(s,re,arr)) so it runs on
# BSD awk (macOS) as well as gawk. Callers run it under LC_ALL=C for byte-stable classes.
...
filenum == 1 {
  ...
  if ($0 ~ /<!--[[:space:]]*adb:override/) {
    ...
    curdir = a; dir_op[a] = o; dir_content[a] = ""; blockbuf = ""; seen_content = 0; in_block = 1; ndir++
    next
  }
...
# ---------- pass 2: the base skill ----------
FNR == 1 && filenum == 2 {                               # opening --- : inject the ownership marker
...
filenum == 2 && frontmatter {                            # stream frontmatter verbatim to its close
  print
  if ($0 == "---") { fmdelims++; if (fmdelims >= 2) frontmatter = 0 }
  next
}
filenum == 2 {
  if ($0 ~ /^```/) { infence = !infence; print; next }   # never treat a fenced line as a heading
  if (!infence && $0 ~ /^### /) {
    if (pending != "") { emit(dir_content[pending]); pending = "" }   # flush prior append
    skipping = 0
    title = $0; sub(/^###[[:space:]]+/, "", title); sub(/^[0-9]+\.[[:space:]]*/, "", title)
    a = slug(title)
...
END {
  ...
  if (filenum < 2) { err("base skill was not read"); exit 3 }
...
# List the base skill's anchors: "<slug>  (line N)  <heading>". Same slug rule as the engine.
# (read -r -d '' for the same backtick-in-bash-3.2 reason as the engine program above.)
IFS= read -r -d '' _ADB_SC_LIST_AWK <<'AWK' || true
function slug(s,   t) { t = tolower(s); gsub(/`/, "", t); gsub(/[^a-z0-9]+/, "-", t); sub(/^-+/, "", t); sub(/-+$/, "", t); return t }
BEGIN { frontmatter = 1; fmdelims = 0; infence = 0 }
NR == 1 { if ($0 == "---") { fmdelims = 1 }; next }
frontmatter { if ($0 == "---") { fmdelims++; if (fmdelims >= 2) frontmatter = 0 }; next }
/^```/ { infence = !infence; next }
!infence && /^### / {
  h = $0; sub(/^###[[:space:]]+/, "", h); t = h; sub(/^[0-9]+\.[[:space:]]*/, "", t)
  printf "%-52s (line %d)  %s\n", slug(t), NR, h
}
AWK
...
adb_sc_render() {
  ...
  if ! LC_ALL=C awk -v ops="$_ADB_SC_OPS" -v agent="$_ADB_SC_AGENT" \
        -v skillname="$name" -v marker="$_ADB_SC_MARKER" -v version="$_ADB_SC_VERSION" \
        "$_ADB_SC_AWK" "$ov" "$base" > "$tmp"; then
    return 1
  fi
  # Self-validate: still a valid SKILL.md, and no leftover directive/placeholder residue.
  if [ "$(head -n1 "$tmp")" != "---" ]; then
  ...
  if grep -Fq '{{' "$tmp"; then
    adb_sc_err "composed output for '$name' contains an unresolved '{{' placeholder"; return 1
  fi
  return 0
}
...
adb_sc_usage() { awk 'NR==1 { next } /^#/ { sub(/^# ?/, ""); print; next } { exit }' "${BASH_SOURCE[0]:-$0}"; }
...
adb_sc_main() {
  ...
  if [ "$cmd" = "list-anchors" ]; then
    ...
    [ -f "$_sc_base" ] || { adb_sc_err "no installed base skill: $_sc_base"; return 1; }
    LC_ALL=C awk "$_ADB_SC_LIST_AWK" "$_sc_base"
    return 0
  fi
