...
Read on demand from step 8. The prompt itself is **built by `{{IMPLEMENT_LIB}} dispatch-review`**
...
omitted. `{{IMPLEMENT_LIB}} review-verdict <file>` is the one reader; `dispatch-review` calls it
...
`{{ROLE_DISPATCH}} review-rung {{CURRENT_AGENT}}` decides it once, over three readers
...
Resolve once — `{{ROLE_DISPATCH}} effort review` — and pass `--effort` to every slot, because
...
2. **Adversarial bug review** — a Claude subagent (Agent tool, `general-purpose`) over the fresh
   ...
   **Then publish its reply — do not consume it directly** (#488):
   ...
   ```bash
   {{IMPLEMENT_LIB}} publish-review [--slot N] {{STATE_DIR}}   # the subagent's reply on stdin
   ```
