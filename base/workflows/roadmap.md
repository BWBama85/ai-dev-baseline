...
re-derived in prose: `{{ROADMAP_LIB}} emit-verdict` takes the still-open members' classification
...
4. **The branch must be green (#78).** A drained checklist says the *requirements* are done; it
   ...
   - **not-green** → withhold the cut and name the failing check. Normally a `/debug` signal —
     **but classify the check before calling it one (#300, D58).** A job that never acquired a
     ...
     {{CI_HEALTH_LIB}} classify --run <id>   # 23 = nothing that failed executed a step
     ```
...
{{ROADMAP_LIB}} slug-ok "$REPO" || exit 1   # #218: API-supplied, and every read below builds `repos/$REPO/...`
...
if [ "$VERDICT" = "met" ]; then
  ...
  REQ_CONTEXTS="$(printf '%s' "$BRANCH_JSON" | {{REPO_SETTINGS_LIB}} branch-required-contexts)" \
  ...
  WF_COUNT=0
  if [ "$(printf '%s' "$HEALTH_IN" | jq '[.check_runs[] | select((.app.slug // "") == "{{ACTIONS_APP_SLUG}}")] | length')" = "0" ]; then
  ...
  OPTOUT_RAW="$(printf '%s' "$ART_JSON" | jq -r '.body // ""' | {{ROADMAP_LIB}} health-optout)" \
  ...
  DECL_OUT="$({{ROADMAP_LIB}} health-decl "$OPTOUT_RAW" "$ART_PERM")" \
...
EOF
  ...
  HEALTH_OUT="$(printf '%s' "$HEALTH_IN" | {{ROADMAP_LIB}} branch-health "$HEAD_SHA" "$WF_COUNT" "$HEALTH_DECL")" \
...
- `not-green` → requirements are met but the branch is **red**. Withhold the cut and name the
  ...
  CI_CLASS=0
  if [ -n "$CI_RUN" ]; then
    {{CI_HEALTH_LIB}} classify --run "$CI_RUN" >/dev/null 2>&1; CI_CLASS=$?
  fi
  ```
...
- **Met** (armed, predicate satisfied, no unacknowledged canceled blocker, **and the branch is
  ...
  **Resolve it before emitting it, and never invent one (#188).** An unresolvable slash command
  ...
  # RENDERED per agent ({{SKILL_PREFIX}}): Claude and Antigravity use a slash command, Codex uses
  ...
  case "$CMD" in
    "$PFX"?*)
      ...
      # {{SKILL_REGISTRY_PROBE}} is empty for agents with no such command; the frontmatter contract
      ...
      if [ -n "$SKILL_NAME" ] && [ "$RESOLVES" -eq 0 ] && [ -z "$PROBE" ]; then
        ...
        # {{SKILLS_SUBDIRS}} is a LIST (Codex loads both `.codex/skills` and `.agents/skills`) and is
...
# repos, many of which don't gitignore {{STATE_DIR}}/, and an untracked file there dirties the
...
if [ -n "$LABEL" ]; then
  ...
  {{ROADMAP_LIB}} slug-ok "$REPO" || exit 1
