...
Address and resolve every unresolved **bot-authored** review thread on the PR named by `{{ARGS}}` so the repo's "all comments must be resolved" branch protection releases. The PR number is the **first bare integer** in those arguments — the rest may carry flags such as `--watch` (step 0).
...
if [ "$WATCH" = "1" ]; then
  ...
  {{PR_WATCH_LIB}} wait --pr "$PR_NUM" --max-secs 540
else
...
: "${BOT_RE:=(?i)^($({{ROLE_DISPATCH}} bots | sed 's/[][]/\\&/g' | paste -sd'|' -))$}"
