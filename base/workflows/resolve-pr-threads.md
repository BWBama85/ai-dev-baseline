...
Wait for the async reviewer, then address and resolve every unresolved **bot-authored** review thread on the PR so the repo's "all comments must be resolved" branch protection releases — and go round again until the reviewer is satisfied. `{{ARGS}}` may be **empty**: the PR number is the first bare integer in it, and when there is none the PR is **inferred** (step 0).
...
(see `base/roles.md`) and read via `{{ROLE_DISPATCH}} bots`: a repo lists/extends it there, leaves
...
if [ -z "$PR_NUM" ]; then
  ...
  PR_NUM="$({{PR_THREADS_LIB}} infer-pr)" || {
...
if [ -z "${MAX_ROUNDS:-}" ]; then
  {{ROLE_DISPATCH}} max-rounds >/dev/null; _mrc=$?
  case "$_mrc" in
...
**The wait itself is free** — `{{PR_WATCH_LIB}} wait` is a `sleep` loop in one process, with no
...
```bash
{{PR_WATCH_LIB}} wait --pr "$PR_NUM" --interval 30 --max-secs 1800
```
...
```bash
{{PR_WATCH_LIB}} wait --pr "$PR_NUM" --interval 30 --max-secs 540
```
...
```
{{PR_WATCH_LIB}} wait --pr N --interval 30 --max-secs 1800
```
...
[ "$_want_rounds" = "1" ] && { echo "ERROR: --max-rounds needs a value"; exit 1; }
if [ -z "$PR_NUM" ]; then
  PR_NUM="$({{PR_THREADS_LIB}} infer-pr)" || { echo "ERROR: could not determine which PR to resolve (see above)"; exit 1; }
fi
...
# empty-regex trap between them. `{{PR_THREADS_LIB}} list` now emits a per-thread `is_bot` computed
# from the same manifest read, so the rule has one home and that home has tests.
if ! KNOWN_BOTS="$({{ROLE_DISPATCH}} bots)"; then
...
```bash
{{PR_THREADS_LIB}} list --pr "$PR_NUM" > {{STATE_DIR}}/threads-$PR_NUM.json
```
...
For every thread with `isResolved: false`, read it with the Read tool (load `{{STATE_DIR}}/threads-$PR_NUM.json`) and decide one of. **A thread already `isResolved` is skipped silently** — that is what makes a second run of this skill a no-op.
...
**Read `is_bot`; do not re-derive it.** The field is computed by `{{PR_THREADS_LIB}}` from the same
...
case "$RUNG" in
  independent*|same-model*)
    {{IMPLEMENT_LIB}} dispatch-review ${EFFORT:+--effort "$EFFORT"} --criteria-from-pr "$PR_NUM" {{STATE_DIR}} "$REVIEW_TOKEN" ;;
...
```bash
{{IMPLEMENT_LIB}} read-artifact {{STATE_DIR}} review
```
...
```bash
{{IMPLEMENT_LIB}} sweep-mark "$SWEEP_FILE" --class <class> --site <path[:line]> --result fixed --fix "$FIX_SHA"
{{IMPLEMENT_LIB}} sweep-mark "$SWEEP_FILE" --class <class> --site <path[:line]> --result deferred --issue <n>
{{IMPLEMENT_LIB}} sweep-mark "$SWEEP_FILE" --class <class> --site <path[:line]> --result declined --reason '<why it differs>'
```
...
- **EVERY result except `0` and `10` STOPS THE ROUND — do not continue to step 5.** A hit that was
  ...
  | `18` | the ledger, or the sweep file, does not parse | **stop**; `{{PATTERN_LEDGER_LIB}} verify` names a ledger record |
...
```bash
REMAINING="$({{PR_THREADS_LIB}} remaining --pr "$PR_NUM")"; RC=$?
```
...
`{{STATE_ASSERT_LIB}} observe pr "$PR_NUM"` (`base/practices/verify-before-asserting.md`).
...
if [ -n "${MAX_ROUNDS:-}" ]; then
  {{PR_WATCH_LIB}} request-review --pr "$PR_NUM" --max-rounds "$MAX_ROUNDS"
else
  {{PR_WATCH_LIB}} request-review --pr "$PR_NUM"
fi
```
...
- **The thread enumeration cannot be proved complete** (`{{PR_THREADS_LIB}}` exit `19`) → abort, run
