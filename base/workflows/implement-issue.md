...
# `{{STATE_DIR}}/review-*.md` to the `rm -f` below looks equivalent and is not — an unmatched glob
...
CLEAR_RC=0
find {{STATE_DIR}} -maxdepth 1 \( -type f -o -type l \) \
     \( -name 'review-*.md' -o -name 'review-*.err' \) -exec rm -f {} + || CLEAR_RC=$?
rm -f {{STATE_DIR}}/review-prompt.txt {{STATE_DIR}}/review.md {{STATE_DIR}}/review.err || CLEAR_RC=$?
[ "$CLEAR_RC" -eq 0 ] || { echo "ERROR: could not clear stale review artifacts from {{STATE_DIR}}"; exit 1; }
```
...
- **On any path that STOPS the run here** — a BLOCKING finding you surface, or a gap-analysis
  ...
  ```bash
  rm -f {{STATE_DIR}}/gap-analysis.lock
  ```
