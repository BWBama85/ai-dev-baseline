...
{{SKILLS_USER_ROOT}}/implement-issue/): `state-protocol.md` — marker/claim/owner contracts in
...
{{STATE_DIR}} <n>…`, dispatch the subagent over it, and write its returned summary to
`{{STATE_DIR}}/survey.md` yourself. **Otherwise**, one bounded background call:
...
- `{{SUBTASK_PRIMITIVE}}` 3–8 tracked sub-tasks. Read `survey.md` first; read code before
