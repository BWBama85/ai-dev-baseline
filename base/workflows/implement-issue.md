...
{{DOCS_LIB}} probe-record --server <name> --result usable|degraded|absent \
  --evidence '<what you observed — the call and its result, not "it worked">'

{{DOCS_LIB}} verdict; VRC=$?
case "$VRC" in
...
```bash
{{DOCS_LIB}} consulted --surface '<the API/service/flag>' --rung <1|2|3> \
...
```bash
{{DOCS_LIB}} none-needed --justification '<why every surface here is trivial or already-proven>'
```
...
`{{DOCS_LIB}} report` returns **11** for a run that recorded neither kind. Step 10 and step 11 both
...
**State the documentation disposition, in one line (#422).** Render it with `{{DOCS_LIB}} report`
