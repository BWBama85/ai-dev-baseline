...
| **2 — deferred to the PR layer** | no in-session slot is usable, **but** `{{ROLE_DISPATCH}} bots --declared` lists an async reviewer | mark the slot **deferred**, say so, and proceed to the PR. |
...
- `codex` (the shipped default) → `{{ROLE_DISPATCH}} invoke codex` over the review prompt
...
- `gemini` → `{{ROLE_DISPATCH}} invoke gemini` over the same prompt (it runs `agy -p`).
