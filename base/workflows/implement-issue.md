...
jq --arg reason "<why this is a legitimate stop>" \
   ...
   {{STATE_DIR}}/implement-issue-active.json > {{STATE_DIR}}/.marker.tmp \
  && mv {{STATE_DIR}}/.marker.tmp {{STATE_DIR}}/implement-issue-blocked.json
```
...
for _probe in issue-0.json issue-0.assoc; do
  ...
  echo "ERROR: {{STATE_DIR}}/$_probe would NOT be gitignored, and step 2 is about to write the"
  echo "       untrusted issue body and its provenance label to exactly that path."
  echo "       Add '{{STATE_DIR}}/' to .gitignore (or re-run 'bin/agent-init') and start again."
  exit 1
done
```
...
for n in "${ISSUE_NUMS[@]}"; do
  gh issue view "$n" --json number,title,body,labels,author,comments,milestone,state > "{{STATE_DIR}}/issue-$n.json" \
...
for n in "${ISSUE_NUMS[@]}"; do
  gh api "repos/{owner}/{repo}/issues/$n" --jq '.author_association' > "{{STATE_DIR}}/issue-$n.assoc" \
    || { {{IMPLEMENT_LIB}} release --token "$RUN_CLAIM_TOKEN" {{STATE_DIR}}; echo "ERROR: could not read #$n's author association"; exit 1; }
done
```
...
```bash
for n in "${ISSUE_NUMS[@]}"; do
  st="$(jq -r .state "{{STATE_DIR}}/issue-$n.json")"
  [ "$st" = "OPEN" ] || { {{IMPLEMENT_LIB}} release --token "$RUN_CLAIM_TOKEN" {{STATE_DIR}}
...
CHECKLIST="$({{PATTERN_LEDGER_LIB}} checklist 2>/dev/null || true)"
if [ -n "$CHECKLIST" ]; then
  {
...
for n in "${ISSUE_NUMS[@]}"; do
  ...
  TEXT="$(jq -r --arg assoc "$ASSOC" '
      ...
      | join("\n\n---\n\n")' "{{STATE_DIR}}/issue-$n.json")" \
    || { {{IMPLEMENT_LIB}} release --token "$RUN_CLAIM_TOKEN" {{STATE_DIR}}; echo "ERROR: could not read issue #$n's text"; exit 1; }
  printf '%s' "$TEXT" | {{ROLE_DISPATCH}} untrusted "github-issue #$n" >> {{STATE_DIR}}/gap-prompt.txt \
    || { {{IMPLEMENT_LIB}} release --token "$RUN_CLAIM_TOKEN" {{STATE_DIR}}; echo "ERROR: could not contain issue #$n's text — do NOT fall back to pasting it raw"; exit 1; }
done
...
`{{STATE_DIR}}`, where step 5's marker is what proves the run is live.

#### 5b-i. The MCP preflight — only for servers this repo DECLARES

```bash
REQUIRED="$({{DOCS_LIB}} mcp-required)"; MRC=$?
case "$MRC" in
...
**Every call below passes `--state {{STATE_DIR}}`, and that is not boilerplate.** Without it the
...
{{DOCS_LIB}} probe-record --state {{STATE_DIR}} --server <name> --result usable|degraded|absent \
  --evidence '<what you observed — the call and its result, not "it worked">'

{{DOCS_LIB}} verdict --state {{STATE_DIR}}; VRC=$?
case "$VRC" in
...
```bash
{{DOCS_LIB}} consulted --state {{STATE_DIR}} --surface '<the API/service/flag>' --rung <1|2|3> \
...
```bash
{{DOCS_LIB}} none-needed --state {{STATE_DIR}} --justification '<why every surface here is trivial or already-proven>'
```
...
| the async reviewer on the PR | **a library wait** — `{{PR_WATCH_LIB}} wait`, driven by `/resolve-pr-threads`, whose step 0b specifies the dispatch |
| the project's gates | **a blocking command** — `{{GATE_RUNNER}} run`, above. Fix and re-run |
...
```bash
{{PATTERN_LEDGER_LIB}} checklist; CRC=$?
case "$CRC" in
...
{{ROLE_DISPATCH}} review-rung {{CURRENT_AGENT}}   # the whole ladder, decided once (rc 2 = unknown)
...
cat > {{STATE_DIR}}/review-prompt.txt <<'PROMPT'
...
for n in "${ISSUE_NUMS[@]}"; do
  ...
  printf '%s' "$TEXT" | {{ROLE_DISPATCH}} untrusted "github-issue #$n — acceptance criteria" >> {{STATE_DIR}}/review-prompt.txt \
...
{{ROLE_DISPATCH}} invoke <token> --effort "$EFFORT" \
...
```bash
{{DOCS_LIB}} report --state {{STATE_DIR}}; DRC=$?
case "$DRC" in
...
**State the documentation disposition, in one line (#422).** Render it with `{{DOCS_LIB}} report --state {{STATE_DIR}}`
