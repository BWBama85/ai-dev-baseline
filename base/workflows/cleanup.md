...
- **Merged branches** — local and, on confirmation, remote. The failure mode this exists to
  ...
  branch explicitly**.
- **Resolved run-state** — the gitignored scratch a skill run leaves in `{{STATE_DIR}}`: thread
...
"; continue
  ...
  if ! V="$(printf '%s' "$PRJSON" | {{CLEANUP_LIB}} branch-verdict "$b" "$BASE")"; then
...
if ! SCAN="$({{CLEANUP_LIB}} state-scan "$STATE")"; then
...
if ! SCAN="$({{CLEANUP_LIB}} state-scan --with-identity "$STATE")"; then
...
while IFS="$TABC" read -r kind sfile key ident; do
  case "$kind" in
    ...
    sweep)
      ...
      PRST="$(pr_state "$key")"
      WV="$({{CLEANUP_LIB}} state-verdict sweep "$PRST")" || continue
      [ "$WV" = stale ] || continue
      sweep_file "$sfile" "$ident" "PR $PRST"
      ;;
  esac
...
"
    ...
    other)
      ...
      [ "$({{CLEANUP_LIB}} marker-shape "$sfile")" = marker-shaped ] || continue
...
`{{STATE_ASSERT_LIB}} lint` rejects a status word sharing a sentence with an entity reference,
