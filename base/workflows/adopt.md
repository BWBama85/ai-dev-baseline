...
Argument: `{{ARGS}}` — an optional path (defaults to the current repo), an optional
...
Every decision above is made by one of two tested predicate libraries — `{{ADOPT_LIB}}`
(`scripts/lib/adopt-lib.sh`, regression-tested by `scripts/check-adopt.sh`) for 1–5, and
`{{ADOPT_READINESS}}` (`scripts/lib/adopt-readiness.sh`, regression-tested by
...
```bash
# ADB-SNIPPET: resolve
# THROUGH {{ADOPT_LIB}}, never by sourcing an agent's common.sh directly. This skill renders for
...
# resolution (base/practices/repo-scope.md) — and `{{ADOPT_LIB}}` is already mapped per agent.
#
...
SHAPE="$({{ADOPT_LIB}} resolve "${TARGET:-$PWD}")"
printf '%s' "$SHAPE" | {{ADOPT_LIB}} shape-field warning | while IFS= read -r m; do
  [ -n "$m" ] && echo "warning: $m" >&2
done
PROJECT="$(printf '%s' "$SHAPE" | {{ADOPT_LIB}} shape-field root)"
...
BASELINE="$({{ADOPT_LIB}} baseline)" || exit 1
{{ADOPT_LIB}} shipped "$BASELINE" > "{{STATE_DIR}}/adopt-shipped.tsv"
...
SHIPPED_N="$(grep -c . "{{STATE_DIR}}/adopt-shipped.tsv" || true)"
[ "${SHIPPED_N:-0}" -gt 0 ] \
...
# ADB-SNIPPET: scan
mkdir -p "{{STATE_DIR}}"
{{ADOPT_LIB}} scan "$PROJECT" ${AGENTS:+--agents "$AGENTS"} > "{{STATE_DIR}}/adopt-scan.tsv"
```
...
`{{GATE_RUNNER}} detect` and never `run`. And when a scanned file's claim bears on the plan
...
- **delta** — `{{ADOPT_LIB}} delta <kind> <project-path> <baseline-path>`. Do **not** substitute
...
- **prescribed** — `{{ADOPT_LIB}} prescribed <kind> <name>` (exit 0 = yes).
...
while IFS="$TABC" read -r kind rel name agent; do
  ...
  if [ "$agent" = "-" ]; then
    src="$(awk -F'\t' -v k="$kind" -v n="$name" '$1==k && $2==n {print $4; exit}' \
             "{{STATE_DIR}}/adopt-shipped.tsv")"
  else
    src="$(awk -F'\t' -v k="$kind" -v n="$name" -v g="$agent" \
             '$1==k && $2==n && $3==g {print $4; exit}' "{{STATE_DIR}}/adopt-shipped.tsv")"
  fi
  coll=no;  [ -n "$src" ] && coll=yes
  presc=no; {{ADOPT_LIB}} prescribed "$kind" "$name" >/dev/null 2>&1 && presc=yes
  delta=unknown
  [ -n "$src" ] && delta="$({{ADOPT_LIB}} delta "$kind" "$PROJECT/$rel" "$src")"
  ...
  line="$({{ADOPT_LIB}} classify "$kind" "$coll" "$delta" "$presc")" || exit 1
  printf '%s\t%s\t%s\t%s\n' "${line%%"$TABC"*}" "$kind" "$rel" "${line#*"$TABC"}"
done < "{{STATE_DIR}}/adopt-scan.tsv" > "{{STATE_DIR}}/adopt-classified.tsv"
```
...
```bash
# ADB-SNIPPET: propose
{{ADOPT_LIB}} roles-infer "$PROJECT" > "{{STATE_DIR}}/adopt-roles.tsv"
{{ADOPT_LIB}} propose < "{{STATE_DIR}}/adopt-roles.tsv" > "{{STATE_DIR}}/adopt-agents.toml"
```
...
Detected gates belong in the same file. `{{GATE_RUNNER}} detect` reports what the ecosystem
...
```bash
# ADB-SNIPPET: hygiene
{{ADOPT_LIB}} hygiene "$PROJECT" ${AGENTS:+--agents "$AGENTS"} > "{{STATE_DIR}}/adopt-hygiene.tsv"
```
...
```bash
# ADB-SNIPPET: pin
{{ADOPT_LIB}} pin-render \
  "$(git -C "$BASELINE" describe --tags --abbrev=0 2>/dev/null || echo '')" \
  "$(git -C "$BASELINE" rev-parse HEAD)" \
  "$(date -u +%Y-%m-%d)" \
  "$({{ADOPT_LIB}} stack "$PROJECT")" \
  "${AGENTS:-claude,codex,gemini}" > "{{STATE_DIR}}/adopt-upstream.toml"
```
...
```bash
# ADB-SNIPPET: drift
{{ADOPT_LIB}} pin-drift .ai-dev-baseline/upstream.toml "$BASELINE"
```
...
```bash
# ADB-SNIPPET: plan
{{ADOPT_LIB}} plan < "{{STATE_DIR}}/adopt-classified.tsv"
```
...
Without `--apply`, everything above is a report and nothing is written outside `{{STATE_DIR}}`.
...
adopt_create "{{STATE_DIR}}/adopt-agents.toml" "$PROJECT/agents.toml" "agents.toml"
...
if [ -L "$PROJECT/.ai-dev-baseline" ]; then
  ...
  adopt_create "{{STATE_DIR}}/adopt-upstream.toml" "$PROJECT/.ai-dev-baseline/upstream.toml" "the upstream pin"
fi
```
...
( cd "$PROJECT" || exit 1
  {{REPO_SETTINGS_LIB}} status      # show what would change BEFORE changing it
  {{REPO_SETTINGS_LIB}} apply )     # required status checks, then allow_auto_merge
```
...
{{ADOPT_READINESS}} receipt run "$PROJECT"
```
...
if [ -n "${AGENTS:-}" ]; then
  {{ADOPT_READINESS}} probe "$PROJECT" --agents "$AGENTS" > "{{STATE_DIR}}/readiness-probe.tsv"
else
  {{ADOPT_READINESS}} probe "$PROJECT" > "{{STATE_DIR}}/readiness-probe.tsv"
fi
```
...
{{ADOPT_READINESS}} tracker < "{{STATE_DIR}}/readiness-facts.json" > "{{STATE_DIR}}/readiness-tracker.tsv"
```
...
cat "{{STATE_DIR}}/readiness-probe.tsv" "{{STATE_DIR}}/readiness-tracker.tsv" \
  | {{ADOPT_READINESS}} verdict
...
baseline adopt status          # or: {{ADOPT_READINESS}} status <project-root>
```
